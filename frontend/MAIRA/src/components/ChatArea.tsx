...
import { Send, FileText, X, Square, Plus, Mic, MicOff, ImageIcon, Loader2, BookOpen, Sparkles, Shield } from "lucide-react";
...
export const ChatArea = () => {
    ...
    // Removed local isTimelineOpen state to use context state
    ...
    // Helper to render the full input bar (used in both empty state and footer)
    const renderInputBar = () => (
        <div className="mx-auto max-w-4xl flex flex-col gap-4 w-full">
            {/* Floating Prompt Bar */}
                ...
                {attachments.length > 0 && (
                    <div className="absolute bottom-full left-0 w-full mb-4 flex flex-wrap gap-2 px-1 z-10">
                        {attachments.map((attachment) => (
                            <div key={attachment.id} className="relative group/attachment flex items-center gap-3 bg-white/90 dark:bg-[#1A1A1A]/90 backdrop-blur-md border border-neutral-200 dark:border-white/10 rounded-xl p-2 pr-8 shadow-lg animate-in fade-in slide-in-from-bottom-2 duration-200">
                                ...
                                <div className="flex flex-col min-w-[60px] max-w-[180px]">
                                    <span className="text-xs text-black dark:text-white truncate font-medium" title={attachment.file.name}>
                ...
                <div className="relative z-20 flex items-center gap-2 rounded-full border border-violet-500/30 bg-[#0A0A0A] p-2 shadow-2xl shadow-violet-900/20 backdrop-blur-xl transition-all duration-300 group-focus-within/input:border-violet-500/80 group-focus-within/input:shadow-violet-500/20">
                    ...
                    <div className="relative">
                        <button
                            ...
                            className={cn(
                                "flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-neutral-100 dark:bg-neutral-800/50 text-neutral-500 dark:text-neutral-400 hover:bg-neutral-200 dark:hover:bg-neutral-800 hover:text-black dark:hover:text-white transition-all active:scale-95",
                                showAttachMenu && "bg-neutral-200 dark:bg-neutral-800 text-black dark:text-white rotate-45"
                            )}
                        ...
                        {showAttachMenu && (
                            <div className="absolute bottom-14 left-0 flex flex-col gap-2 p-2 rounded-xl bg-[#0A0A0A] border border-neutral-800 shadow-xl shadow-black/50 overflow-hidden animate-slide-up origin-bottom-left z-50 min-w-[140px]">
                                <button
                                    ...
                                    className="flex items-center gap-3 px-3 py-2 text-sm text-neutral-600 dark:text-neutral-400 hover:text-black dark:hover:text-white hover:bg-neutral-100 dark:hover:bg-neutral-800/50 rounded-lg transition-colors text-left"
                                >
                                    <div className="p-1.5 rounded-full bg-blue-500/10 text-blue-500 dark:text-blue-400">
                                        <FileText size={16} />
                                    </div>
                                    <span>Document</span>
                                </button>
                                <button
                                    ...
                                    className="flex items-center gap-3 px-3 py-2 text-sm text-neutral-600 dark:text-neutral-400 hover:text-black dark:hover:text-white hover:bg-neutral-100 dark:hover:bg-neutral-800/50 rounded-lg transition-colors text-left"
                                >
                                    <div className="p-1.5 rounded-full bg-purple-500/10 text-purple-500 dark:text-purple-400">
                                        <ImageIcon size={16} />
                                    </div>
                                    <span>Image</span>
                                </button>
                    ...
                    <textarea
                        ...
                        placeholder={isLoading ? "Generating response..." : "Ask MAIRA..."}
                        disabled={isLoading}
                        className="flex-1 resize-none bg-transparent px-2 py-2.5 text-sm text-white placeholder-neutral-500 focus:outline-none font-medium leading-relaxed min-h-[44px] max-h-[120px]"
                        rows={1}
                            ...
                            <button
                                ...
                                className={cn(
                                    ...
                                    isListening
                                        ...
                                        : "text-neutral-400 hover:bg-neutral-100 dark:hover:bg-neutral-800 hover:text-black dark:hover:text-white"
                                )}
            ...
            <div className="flex flex-nowrap items-center justify-between gap-4 px-2">
                ...
                {/* Research Toggles */}
                <div className="flex items-center gap-2 overflow-x-auto pb-1 min-w-0 mask-gradient-right">
                    {/* Deep Research */}
                    <button
                        ...
                        className={cn(
                            ...
                            deepResearch
                                ...
                                : "bg-white/5 border-neutral-200 dark:border-white/5 text-neutral-600 dark:text-neutral-500 hover:bg-neutral-100 dark:hover:bg-white/10 hover:text-black dark:hover:text-white"
                        )}
                    >
                        <Sparkles size={12} />
    ...
    return (
        <div className="flex h-screen flex-1 flex-col bg-black relative overflow-hidden transition-colors duration-300">
            {/* Background Gradient */}
            ...
            <main className="flex-1 overflow-y-auto overflow-x-hidden scroll-smooth">
                <div className="mx-auto max-w-4xl w-full px-6 py-12">
                    {isLoadingMessages ? (
                        ...
                        <div className="flex h-[80vh] flex-col items-start justify-center text-left animate-fade-in relative z-10">
                            {/* Greeting */}
                            <div className="mb-12 space-y-4">
                                <h1 className="text-4xl md:text-6xl font-bold text-white tracking-tight">
                                    <span className="bg-gradient-to-r text-white/50 bg-clip-text text-transparent">
                                        Hi {user?.user_metadata?.full_name?.split(' ')[0] || 'User'}
                                    </span>
                                </h1>
                                <h2 className="text-3xl md:text-5xl font-bold text-white/50 tracking-tight">
                        ...
                        <div className="flex flex-col gap-8">
                            ...
                            {isLoading && deepResearch && currentMessages.some(m => m.type === 'streaming') && (() => {
                                const streamingMsg = currentMessages.find(m => m.type === 'streaming');
                                const steps = Object.values(activeSteps || {}) as any[];
                                const sortedSteps = [...steps].sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0));
                                    ...
                                    <div className="animate-slide-up">
                                        <div className="flex w-full items-start gap-5">
                                            ...
                                            <DeepResearchProgress
                                                isActive={true}
                                                status={sortedSteps[0]?.action || streamingMsg?.status}
                                                statusDetail={sortedSteps[0]?.detail || streamingMsg?.statusDetail}
                                                statusIcon={streamingMsg?.statusIcon}
                                                progress={sortedSteps[0]?.progress ?? streamingMsg?.progress}
                                                currentPhase={(sortedSteps[0]?.phase || streamingMsg?.currentPhase) as any}
                                                phaseName={streamingMsg?.phaseName}
                                                phaseDescription={streamingMsg?.phaseDescription}
                                            />
                                        </div>
                                    </div>
                                );
            ...
            {/* Input Area (Footer) - Only show if messages exist */}
            {currentMessages.length > 0 && (
                <footer className="p-6 md:p-10 relative z-20 mx-auto w-full max-w-4xl">
                    {renderInputBar()}
                </footer>
            )}
